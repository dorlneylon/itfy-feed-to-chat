...
if __name__ == "__main__":
    ...
    def text_com(message):
        user = {}
        request = apiai.ApiAI('YOUR DIALOGFLOW ID GOES HERE').text_request()
        request.lang = 'ru'
        ...
        if message.reply_to_message is not None and message.reply_to_message.from_user.is_bot is not True:  # Check if replied message is replied to a human
            ...
            if str(message.text).startswith('!go'):  # Google
                ...
                if user['username'] in ADMINS:  # Bot checks if user is in admins so randoms can't use this a lot
                    ...
                    try:
                        add_tp_help(log='your google account login goes here', password='your google account password goes here', message=f'{data2input}')
                ...
                if user['username'] in ADMINS:  # Bot checks if user is in admins so randoms can't use this a lot
                    data2input = message.reply_to_message.text
                    try:
                        add_tp_pass(log='your google account login goes here', password='your google account password goes here', message=f'{data2input}')
                    except selenium.common.exceptions.NoSuchElementException:
                        bot.send_message(
                            chat_id=message.chat.id,
                            reply_markup=None,
